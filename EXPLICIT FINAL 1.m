...
if lambda > 0.5
    warning('Stability condition violated: lambda = %f > 0.5', lambda);
end
...
%% Coefficient matrix B for explicit scheme (interior nodes only)
main_diag = (1 - 2*lambda) * ones(Nz-1, 1);
off_diag = lambda * ones(Nz-2, 1);
B = diag(main_diag) + diag(off_diag, 1) + diag(off_diag, -1);
...
title('U_{avg} vs T_v: Explicit Matrix Scheme vs Theoretical');
legend('Numerical (Explicit Matrix)', 'Theoretical');
grid on;
...
ylabel('Depth (m)');
title('Pore Pressure Distribution at Final Time (Explicit Matrix Scheme)');
grid on;