...
%% 
sigma0 = 3.47;
sigma_dash = 16 * sigma0;  % Final stress
...
A(Nz,Nz-1) = 2 * A(Nz,Nz-1);  % For PTIB using central difference formula

for n = 1:Nt
    b = w(2:Nz+1, n);         % Interior nodes from previous time step for PTIB
...
u_max_percent = (u_max / u_max(1)) * 100;       % Normalize to initial and convert to %
...
semilogx(Tv_exp, u_percent_exp, 'ko', ...
    'MarkerFaceColor', 'y', 'MarkerSize', 6);
xlabel('Time factor T_v');
ylabel('Maximum Excess Pore Pressure (% of Initial)');
title('T_v vs Maximum Excess Pore Pressure (σ''_f / σ''_0 = 16)');
legend('Numerical (This Study)', ...
       'Experimental (Davis \& Raymond, 1965)', ...
       'Location', 'northeast');
ylim([0 110]); grid on;